<template>
  <!-- 修改纸板采购单界面 -->
  <div>
    <el-form ref="form" :model="form" style="margin:10px 40px 0px 10px;line-height:5px" label-position="right" label-width="100px">
      <el-row>
        <el-col :span="8">
          <el-form-item>
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item label="采购日期:">
            <el-date-picker v-model="form.dcgrq" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择日期" style="width: 180px;"></el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="到货日期:">
            <el-date-picker v-model="form.ddhrq" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择日期" style="width:215px"></el-date-picker>
          </el-form-item>
        </el-col>
        <el-col :span="8">
...
export default {
  ...
  created() {
    ...
    console.log("进来了"+JSON.stringify(check))
  ...
  methods: {
    ...
    save() {
      for (var i = 0; i < this.suppliers.length; i++) {
        if (this.form.cqybhGys == this.suppliers[i].qymc) {
          this.form.cqybhGys = this.suppliers[i].qybh;
        }
      }
      this.form.cqybh = this.qybh;
      this.axios
        .post('/PaperBoardController/modifyPaperBoard',  this.form )
        .then(response => {
          if (response.data.status === '1') {
